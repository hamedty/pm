...
public:
  ...
  /// The packet handler method usually has the form:
  ///
  ///     void onPacketReceived(void* sender, const uint8_t* buffer, size_t
  // size);
  ///
  ...
  void begin(unsigned long speed)
  {
    ...
    while (!Serial);
        #endif // if ARDUINO >= 100 && !defined(CORE_TEENSY)
    setStream(&Serial);
  }
  ...
  /// The packet handler method usually has the form:
  ///
  ///     void onPacketReceived(const void* sender, const uint8_t* buffer,
  // size_t size);
  ///
  ...
  /// Setting a packet handler will remove all other packet handlers.
  ///
  /// \param onPacketFunctionWithSender A pointer to the packet handler
  // function.
