/*
   ...
   Thanks to stimmer (from Arduino forum), for coding the "timer soul" (Register
      stuff)
...
int DueTimer::getAvailable(void) {
  /*
     Return the first timer with no callback set
   */

  for (int i = 0; i < NUM_TIMERS; i++) {
    if (!callbacks[i]) return i;
  }
...
uint8_t DueTimer::bestClock(double frequency, uint32_t& retRC) {
  ...
  do
  {
    ticks = (float)SystemCoreClock / frequency /
            (float)clockConfig[clkId].divisor;

    // error = abs(ticks - round(ticks));
    error = clockConfig[clkId].divisor * abs(ticks - round(ticks)); // Error
                                                                    // comparison
                                                                    // needs
                                                                    // scaling
  ...
  ticks = (float)SystemCoreClock / frequency /
          (float)clockConfig[bestClock].divisor;
...
DueTimer& DueTimer::setFrequency(double frequency) {
  ...
  // Remember the frequency — see below how the exact frequency is reported
  // instead
